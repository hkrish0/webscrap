 ...
 /* Get all Gk Categories */
	...
	public function addToMountCart($data, $stock_status=0, $quantity=0, $status = 0) {
        ...
        while($product_name_count > 0 ){
            	...
            	$related_products = Yii::app()->db2->createCommand('SELECT product_id FROM `oc_product` WHERE product_id IN (SELECT product_id FROM oc_product_to_category WHERE category_id="'.$categories[1].'") AND product_id IN (SELECT product_id FROM `oc_product_description` WHERE name SOUNDS LIKE "'.$product_keyword.'%") AND product_id IN (SELECT product_id FROM `oc_product_attribute` where text="'.$data['attribute'].'")  LIMIT 10')->queryAll();
